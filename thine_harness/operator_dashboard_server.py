...
async function refresh(){{status.textContent='Refreshing…';try{{const r=await fetch('/api/snapshot');const s=await r.json();panels.innerHTML=Object.entries(s.panels).map(([name,p])=>{{const fresh=p.freshness.observed_at_ms===null?'freshness unknown':`observed ${{new Date(p.freshness.observed_at_ms).toLocaleTimeString()}} (${{p.freshness.age_ms}}ms ago)`;return `<article><h2>${{name.replaceAll('_',' ')}}</h2><div class="meta"><span class="${{p.status}}">${{p.status}}</span> · ${{p.source}} · ${{fresh}}</div>${{p.error?`<p class="error">${{p.error}}</p>`:''}}<pre>${{esc(p.data)}}</pre></article>`}}).join('');status.textContent=`Updated ${{new Date(s.generated_at_ms).toLocaleTimeString()}} · last ${{s.limit}}`;}}catch(e){{status.textContent='Refresh failed: '+e}}}}
