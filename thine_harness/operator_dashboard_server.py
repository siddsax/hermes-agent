...
async function refresh(){{status.textContent='Refreshing…';try{{const r=await fetch('/api/snapshot');const s=await r.json();panels.innerHTML=Object.entries(s.panels).map(([name,p])=>{{const owner=p.freshness.owner_observed_at_ms===null?'owner observation unavailable':`owner observed ${{new Date(p.freshness.owner_observed_at_ms).toLocaleTimeString()}} (${{p.freshness.age_ms}}ms ago)`;const read=`read ${{new Date(p.freshness.read_at_ms).toLocaleTimeString()}}`;return `<article><h2>${{name.replaceAll('_',' ')}}</h2><div class="meta"><span class="${{p.status}}">${{p.status}}</span> · ${{p.source}} · ${{read}} · ${{owner}}</div>${{p.error?`<p class="error">${{p.error}}</p>`:''}}<details><summary>source freshness</summary><pre>${{esc(p.freshness.components)}}</pre></details><pre>${{esc(p.data)}}</pre></article>`}}).join('');status.textContent=`Updated ${{new Date(s.generated_at_ms).toLocaleTimeString()}} · last ${{s.limit}}`;}}catch(e){{status.textContent='Refresh failed: '+e}}}}
